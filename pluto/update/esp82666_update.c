...
static char *EXAMPLE_SERVER_IP;
static char *EXAMPLE_SERVER_PORT;
static char *EXAMPLE_FILENAME;
...
static void __attribute__((noreturn)) task_fatal_error()
{
    ...
    close(socket_id);
    esp_udpate_finished_cb(ES_FAILED);
...
static void ota_example_task(void *pvParameter)
{
    ...
    while (flag) {
        ...
        if (buff_len < 0) { /*receive error*/
            ...
            eloop_log(1, "written image length %d\r\n", binary_file_length);
...
{
    uint8 *pdata;
    if((pdata = eloop_malloc(1024))!=NULL)
    {
        eloop_memset(pdata,0,1024);
        EXAMPLE_SERVER_IP = (char*)&pdata[0];
        EXAMPLE_SERVER_PORT = (char*)&pdata[128];
        EXAMPLE_FILENAME = (char*)&pdata[256];
        eloop_log(3,"esp_update_start:ip:%s:%d,%s\r\n",ip,port,url);
        eloop_strcpy(EXAMPLE_SERVER_IP,ip);
        eloop_strcpy(EXAMPLE_FILENAME,url);
        EXAMPLE_SERVER_PORT = eloop_itoa(port,EXAMPLE_SERVER_PORT);
        xTaskCreate(&ota_example_task, "ota_example_task", 4096, NULL, 5, NULL);
    }
}
