...
{
  eloop_log(DBG_HAL_LED,"hal_lamp_set:lamps:%02x val:%d \r\n",lamps,state);
  if(lamps&LAMP0)
  {
    if(state==LAMP_ON)
    	set_lamp0();
...
{
	eloop_log(DBG_HAL_LED,"hal_lamp_get:lamps:%02x \r\n",lamps);
  if(lamps&LAMP0)
  {
